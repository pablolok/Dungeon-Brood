...
echo Running Dungeon Brood mouse aim smoke test with "%UE_ROOT%"
"%UE_ROOT%\Engine\Binaries\Win64\UnrealEditor.exe" "%PROJECT%" -game -nullrhi -nosound -unattended -ExecCmds="BroodMouseAimSmokeTest"
if errorlevel 1 (
    echo.
    echo ERROR: Mouse aim smoke test failed.
    exit /b 1
)

powershell -NoProfile -ExecutionPolicy Bypass -Command "$log=Get-Content -Raw '%~dp0Saved\Logs\DungeonBrood.log'; if($log -notmatch 'BROOD_MOUSE_AIM_SMOKE_PASSED' -or $log -match 'BROOD_MOUSE_AIM_SMOKE_FAILED'){exit 1}"
if errorlevel 1 (
    echo.
    echo ERROR: Mouse aim smoke test runtime marker was not successful.
    exit /b 1
)
