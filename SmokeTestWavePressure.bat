...
echo Running Dungeon Brood wave pressure smoke test with "%UE_ROOT%"
"%UE_ROOT%\Engine\Binaries\Win64\UnrealEditor.exe" "%PROJECT%" -game -nullrhi -nosound -unattended -ExecCmds="BroodWavePressureSmokeTest"
if errorlevel 1 (
    echo.
    echo ERROR: Wave pressure smoke test failed.
    exit /b 1
)

powershell -NoProfile -ExecutionPolicy Bypass -Command "$log=Get-Content -Raw '%~dp0Saved\Logs\DungeonBrood.log'; if($log -notmatch 'BROOD_WAVE_PRESSURE_SMOKE_PASSED' -or $log -match 'BROOD_WAVE_PRESSURE_SMOKE_FAILED'){exit 1}"
if errorlevel 1 (
    echo.
    echo ERROR: Wave pressure smoke test runtime marker was not successful.
    exit /b 1
)
