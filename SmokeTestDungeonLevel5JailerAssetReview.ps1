...
$screenshotPath = Join-Path $workspace "Saved\Automation\WindowedJailerHuskAssetReviewViewport.png"
...
try {
    $arguments = @(
        ...
        "-WinX=140",
        "-WinY=140",
        "-ExecCmds=BroodDungeonLevel5JailerAssetReviewSmokeTest"
    )
    $unreal = Start-Process -FilePath $unrealEditor -ArgumentList $arguments -WorkingDirectory $workspace -PassThru

    Wait-LogPattern "BROOD_DUNGEON_LEVEL5_JAILER_HUSK_ASSET_REVIEW_STARTED"
    Wait-LogPattern "BROOD_DUNGEON_LEVEL5_JAILER_HUSK_ASSET_REVIEW_TURNAROUND_READY"
    Wait-LogPattern "BROOD_DUNGEON_LEVEL5_JAILER_HUSK_ASSET_REVIEW_CLOSED_STAGE_READY"
    Wait-LogPattern "BROOD_DUNGEON_LEVEL5_JAILER_HUSK_ASSET_REVIEW_LIGHTING_READY"
    ...
    Wait-LogPattern "BROOD_DUNGEON_LEVEL5_JAILER_HUSK_ASSET_REVIEW_SCREENSHOT_REQUESTED"
    Wait-LogPattern "BROOD_DUNGEON_LEVEL5_JAILER_HUSK_ASSET_REVIEW_PASSED"
    ...
    if (-not (Test-Path $screenshotPath) -or (Get-Item $screenshotPath).Length -le 0) {
        throw "The Jailer Husk asset review screenshot was not written: $screenshotPath"
    }

    Write-Host "Dungeon Level 5 Jailer Husk asset review passed: screenshot saved to $screenshotPath"
}
finally {
