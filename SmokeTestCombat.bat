...
echo Running Dungeon Brood combat smoke test with "%UE_ROOT%"
"%UE_ROOT%\Engine\Binaries\Win64\UnrealEditor.exe" "%PROJECT%" -game -nullrhi -nosound -unattended -ExecCmds="BroodCombatSmokeTest"
if errorlevel 1 (
    echo.
    echo ERROR: Combat smoke test failed.
    exit /b 1
)

powershell -NoProfile -ExecutionPolicy Bypass -Command "$log=Get-Content -Raw '%~dp0Saved\Logs\DungeonBrood.log'; if($log -notmatch 'BROOD_COMBAT_SMOKE_DROP_VERIFIED' -or $log -notmatch 'BROOD_COMBAT_SMOKE_PASSED' -or $log -match 'BROOD_COMBAT_SMOKE_FAILED'){exit 1}"
if errorlevel 1 (
    echo.
    echo ERROR: Combat smoke test runtime marker was not successful.
    exit /b 1
)
